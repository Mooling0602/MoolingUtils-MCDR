...
def tr(server: PluginServerInterface, tr_key: str, return_str: Optional[bool] = True):
    '''
    [zh_CN]
    ...
    if tr_key.startswith(f"{plgSelf.id}"):
        ...
        if tr_key.startswith("#"):
            translation = server.rtr(tr_key.replace("#", ""))
        else:
            translation = server.rtr(f"{plgSelf.id}.{tr_key}")
    if return_str:
